...
func TestVersionedRelationKeepsVerdictOnEvidence(t *testing.T) {
	svc := newTestService(t)
	batch, _ := svc.CreateBatch("version-batch", "")
	...
	if got.Verdict != model.VerdictConfirmed {
		t.Fatalf("versioned relation must keep confirmed, got %s", got.Verdict)
	}
}
