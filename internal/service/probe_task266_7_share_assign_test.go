...
func TestShareVersionAssignsRelationOwnership(t *testing.T) {
	svc := newTestService(t)
	batch, _ := svc.CreateBatch("share-batch", "")
	...
	ver, err := svc.CreateVersion("v-share", "")
	if err != nil {
		t.Fatal(err)
	}
	if _, err := svc.ShareVersion(ver.ID); err != nil {
		t.Fatal(err)
	}
	...
	if got.VersionID != ver.ID {
		t.Fatalf("version_id = %d, want %d", got.VersionID, ver.ID)
	}
}
